...
-- @datetime 2022.10.22
-- @desc 数据隔离型的即去数据化
local ActivityIslandMapBehaviour          = class(BaseBehaviour, "ActivityIslandMapBehaviour")
...
-- 地图初始化，在Awake之前
function ActivityIslandMapBehaviour:init(...) 
    -- param init
    local param = {...}
    local mapPath = ""
    if param[2] then
        ...
        mapPath = string.split(mapArr[#mapArr], "%.")[1]
    end
    if param[3] then
    ...
    self.FirstEnter = UPlayerPrefs.GetString(CURRENT_USER .. "_" .. LocalData.First .. "_" .. mapPath .. "V2", "") == ""
end

function ActivityIslandMapBehaviour:Awake()
        ...
        -- if self.model then
            ...
            self:LoadStage(function()
                ...
                App:applyFunc(ConstData.Popup, "loadAsync", function() 
                    ...
                    -- if lvType == ConstData.STR_MASTER then 这里是做进入游戏后得处理，如story场景得教学， 活动岛需要得话，回改为发送一个事件
                    -- end
...
-- 加载预设
function ActivityIslandMapBehaviour:PreloadSceneElements()
    -- 普通Element 可以回收
...
-- 加载通用Stage
function ActivityIslandMapBehaviour:LoadStage(callback)
...
function ActivityIslandMapBehaviour:LoadLevelMap(level, callback)
    -- 使用coroutine
...
function ActivityIslandMapBehaviour:MapInit()
    -- setStartPosition
...
function ActivityIslandMapBehaviour:ObjectsInit()
...
-- 初始化地图矩阵
function ActivityIslandMapBehaviour:initMartix()
    -- 判断是否有当前缓存的数据，针对剧情关卡
    local mapArr = string.split(self.levelMap, "/")
...
-- func 进入地图打点
function ActivityIslandMapBehaviour:dcEnterMap(isFirst, mapPath)
    local data = {
        label_id = mapPath,
...
-- 添加npc
function ActivityIslandMapBehaviour:AddNpc(cellId, npcElement)
...
-- 加载当前地图的数据 
function ActivityIslandMapBehaviour:startLoadMap()
    ...
    coroutine.waitforframes(FRAME_ONE)
    App:applyFunc(ConstData.Cloud, "setAllCloudFlag") -- 刷新云朵数据 -- 这里需要好好看看，目前是存在云朵的bug的
...
-- 新地图加载
function ActivityIslandMapBehaviour:startNewMap(mapPath)
...
function ActivityIslandMapBehaviour:checkWall(x, y)
    return false
end

-- 拖拽 此处特别说明 current表示CommonMap, self表示gameobject所属的Behaviour 
function ActivityIslandMapBehaviour:DragBegin(gameObject, position)
...
function ActivityIslandMapBehaviour:DragMoving(gameObject, position)
    local cond = 
    ...
    if not ((distance_tmp and distance_tmp < 0.1) and (current.maxDistance and current.maxDistance < 0.5)) then
        ...
        if not self.popup_hide_all then
            ...
            App:applyFunc(ConstData.ActivityMapMsg, "notify", ConstData.MapEvents.ON_DRAG, {from = "DragBegin", param = self})
        end
    end
...
-- 防止死循环
-- 2022 袁博备注，点击障碍物
function ActivityIslandMapBehaviour:OnClick(gameObject, position, isNotMoved, _currentTiled, force, dialogForce)
    ...
    if diff_distance < 0.5 then
        ...
        App:applyFunc(ConstData.ActivityMapMsg, "notify", ConstData.MapEvents.ELEMENT_CLICK, {from = "MapElementClick", params = {ele = self}, posGroup = {current.beginPosition, Vector3.zero}})
    end
end
...
-- dialogForce这里的dialogForce, 判断调用是否来自dialog和其他元素重叠时，优先处理dialog,若dialogForce为true,则，不再进行重叠检测
-- 防止死循环
function ActivityIslandMapBehaviour:DragEnded(gameObject, position, isNotMoved, _currentTiled, force, dialogForce, isMov)
    ...
    if ((distance_tmp and distance_tmp < 0.1) and (current.maxDistance and current.maxDistance <= 0.12)) or dialogForce then
        ...
        App:applyFunc(ConstData.ActivityMapMsg, "notify", ConstData.MapEvents.END_DRAG, {from = "DragEnded-line1", param = self})
        local touchStatus = App:applyFunc(ConstData.Touch, "getStatus")
        ...
        if self.data and touchStatus == touchList.COMMON and self.tag ~= "MoveOnly" then
            ...
            App:applyFunc(ConstData.ActivityMapMsg, "notify", ConstData.MapEvents.ELEMENT_CLICK, {from = "MapElementClick", params = {ele = self}, posGroup = {current.beginPosition, vec_world}})
            if self.data.ItemType == "npc" then
                ...
                if not self.data["Behaviour"] or self.data["Behaviour"] == '' then
                    ...
                    local noBanner = true
                    if App:applyFunc(ConstData.FirstPlayTeach, "CheckTeached", "Ph05_Free_Gift_07") then 
    ...
    if type(currentTiled) == ConstData.TABLE_TYPE and not currentTiled.data then
        self:BounceBack()
        return
    end
    local IsObstacle = nil
    ...
    if self.data and (not currentTiled or (type(currentTiled) == ConstData.TABLE_TYPE and currentTiled.data and not table.indexof(StoryMarkData, currentTiled.data.ItemType))) then
        -- 普通类
            ...
            if current.currentMatrix[vec.x + 1 + vec.y * current.cellXCount] ~= "" then
                ...
                if not isForce then
                    ...
                    AnimateTween.setCallBack(aid, function() 
                        ...
                        App:applyFunc(ConstData.ActivityMapMsg, "notify", ConstData.MapEvents.END_DRAG, {from = "DragEnded-line2", param = self, pos = newPosition})
                    end)
                else
                    gameObject.transform.position = newPosition
                    App:applyFunc(ConstData.ActivityMapMsg, "notify", ConstData.MapEvents.END_DRAG, {from = "DragEnded-line3", param = self})
                end
        ...
        elseif currentTiled and (type(currentTiled) == ConstData.TABLE_TYPE and self.data.ItemType == "timeelf") or ((currentTiled == "") and not is_lock and self.data.ItemType == "timeelf") then
            if  (type(currentTiled) == ConstData.TABLE_TYPE and self.data.ItemType == "timeelf" and (currentTiled.status == StatusConst.WaitingTimeForBuild or currentTiled.status == StatusConst.WaitingForBuild or currentTiled.status == StatusConst.UnBuild)) then
                                ...
                                if FakeCorouteInst then
                                    FakeCorouteInst:CreateFakeCoroute(FRAME_ONE, function() 
                                        App:applyFunc(ConstData.Time,  "quickTime", self, currentTiled, function(overall) 
                                            ...
                                            App:applyFunc(ConstData.ActivityMapMsg, "notify", ConstData.MapEvents.ONCE_TASK, {from = "ActivityIslandMap", param = {"UseWorker-1"}})
                                            ...
                                            if overall then
                                                ...
                                                local aid = AnimateTween.moveToV3(self.gameObject, newPosition, 0.33)
                                                AnimateTween.scaleTo(self.gameObject, Vector3.one, 0.33)
                                                aid:setEase(LeanTweenType.easeOutQuint)
                                                AnimateTween.setCallBack(aid, function() 
                                                    self.gameObject.transform.position = newPosition
                                                    App:applyFunc(ConstData.ActivityMapMsg, "notify", ConstData.MapEvents.END_DRAG, {from = "DragEnded-line4", param = self, pos = newPosition})
                                                end)
                                                self:setNeedSortingOrder(true)
                                            end
                                        end)
                ...
                and not current:checkWall(vec.x, vec.y) then
                    ...
                    AnimateTween.setCallBack(aid, function() 
                        gameObject.transform.position = newPosition
                        App:applyFunc(ConstData.ActivityMapMsg, "notify", ConstData.MapEvents.END_DRAG, {from = "DragEnded-line5", param = self, pos = newPosition})
                    end)
                ...
                elseif type(other) == ConstData.TABLE_TYPE then
                    ...
                    AnimateTween.setCallBack(aid, function() 
                        self.gameObject.transform.position = newPosition
                        App:applyFunc(ConstData.ActivityMapMsg, "notify", ConstData.MapEvents.END_DRAG, {from = "DragEnded-line6", param = self, pos = newPosition})
                    end)
                    local index = self.X + 1 + self.Y * current.cellXCount
                    current.emptyMatrix[index] = true
                    current.currentMatrix[index] = ""
    ...
    elseif self.data and (not currentTiled or (type(currentTiled) == ConstData.TABLE_TYPE and not table.indexof(StoryMarkData, currentTiled.data.ItemType))) then
        self.data.Grid = {2, 2}
        local grid = self.data.Grid
        local tempGrids = {}
        local IsObstacle = nil
        ...
        if not is_B and not is_C and not IsObstacle and not is_lock and is_Y and not bigBuilder then
            ...
            AnimateTween.setCallBack(aid, function() 
                self.gameObject.transform.position = newPosition
                App:applyFunc(ConstData.ActivityMapMsg, "notify", ConstData.MapEvents.END_DRAG, {from = "DragEnded-line7", param = self, pos = newPosition})
            end)
            self.X = vec.x
            self.Y = vec.y - 1
            self:setNeedSortingOrder(true)
    ...
    elseif (currentTiled or (type(currentTiled) == ConstData.TABLE_TYPE and table.indexof(StoryMarkData, currentTiled.data.ItemType))) then
        ...
        if currentTiled == "" then
            ...
            if not isForce then
                ...
                AnimateTween.setCallBack(aid, function() 
                    gameObject.transform.position = newPosition
                    App:applyFunc(ConstData.ActivityMapMsg, "notify", ConstData.MapEvents.END_DRAG, {from = "DragEnded-line10", param = self, pos = newPosition})
                end)
            else
                gameObject.transform.position = newPosition
                App:applyFunc(ConstData.ActivityMapMsg, "notify", ConstData.MapEvents.END_DRAG, {from = "DragEnded-line11", param = self})
            end
            local index = vec.x + 1 + vec.y * current.cellXCount
            current.emptyMatrix[index] = false
            current.currentMatrix[index] = self
        ...
        local aid = AnimateTween.moveToLocal3(self.gameObject, self.originPosition, 0.25)
        AnimateTween.setCallBack(aid, function() 
            if self.hideObject then
                self.hideObject.gameObject:SetActive(true)
            end
        end)
    end
...
function ActivityIslandMapBehaviour:IsLastNpcToken(tokenCellId)
...
function ActivityIslandMapBehaviour:OnEnable()

end

function ActivityIslandMapBehaviour:Update()
    if current.interactTime then
        current.interactTime = current.interactTime + DeltaTime
    end
end

function ActivityIslandMapBehaviour:LateUpdate()  
    -- 是否生成气泡
    ...
    if Input.touchCount == 1 or CS.UnityEngine.Application.isEditor or ExtDrag.Drag then
        ...
        if Input.GetMouseButtonDown(0) then
            self.lastClick = nil
            self.mov_vec = 0
            self:RefreshStartPosPointerDown()
...
function ActivityIslandMapBehaviour:InertiaSpeedMove()
    if self.speed then
...
function ActivityIslandMapBehaviour:Start()
end

function ActivityIslandMapBehaviour:OnDisable()
    
end

function ActivityIslandMapBehaviour:OnDestroy()
    debug.log("MapBehaviour ---> OnDestroy!!!")
end

function ActivityIslandMapBehaviour:onLevelScene()
    -- 退出进行存储
...
------------------------------------------ 测试 -----------------------------------------
--- 测试
function ActivityIslandMapBehaviour:DelayCallTest()
    -- 测试delayCall [OK]
...
function ActivityIslandMapBehaviour:SoundTest()
    -- 测试声音
...
function ActivityIslandMapBehaviour:JudgeBanner(element, ret_status)
    if not IsNil(element) and not ret_status then
        ...
        elseif element.tag == "AwardTag" then 
            element:ClickCreateAward()
        end
    elseif element.tag == "AwardTag" then
        element:ClickCreateAward()
    end
...
function ActivityIslandMapBehaviour:ObstacleForceClick(element)
...
function ActivityIslandMapBehaviour:ObstacleForceNoRetClick(element)
    element:shake()
    if element.status == StatusConst.WaitingTimeForBuild or 
        ...
        if element.cost_time and element.cost_time > 31 then
            UIManager:GetInstance():OpenWindow(Config.UIWindowNames.UINotice01RefreshPannel, {"expend"}, element, App:applyFunc(ConstData.AdsPlay,  "UsingAdsPannel", element))
        end
        App:applyFunc(ConstData.Sound,  "playSound", "touch")
        return
    end
...
-- 交换两个元素的代码抽离
function ActivityIslandMapBehaviour:ExchangeTwoElement(other, element)
    -- App:applyFunc(ConstData.MapCore, "RemoveGrid", 1)
    -- 位置上的物体移动到vv1
...
-- 就近找，挤走等代码抽离
function ActivityIslandMapBehaviour:MoveToNeatBy(element, vec, other, cellHeight)
    ...
    if next_pos and #next_pos > 0 then
        ...
        local oldPosition = other.gameObject.transform.position
        local newPosition = Vector3(vv1.x, vv1.y - cellHeight, -0.005)
        local other_aid = AnimateTween.moveToV3Update(other.gameObject, newPosition, 0.33, function(pos)
            other:UpdateBannerPos(pos)
        end)
        other_aid:setEase(LeanTweenType.easeOutQuint)
        AnimateTween.setCallBack(other_aid, function() 
            other.gameObject.transform.position = newPosition
        ...
        AnimateTween.setCallBack(cur_aid, function() 
            gameObject.transform.position = newPosition
            App:applyFunc(ConstData.ActivityMapMsg, "notify", ConstData.MapEvents.END_DRAG, {from = "DragEnded-line14", param = element, pos = newPosition})
        end)
...
-- 平地移动时相机也移动
function ActivityIslandMapBehaviour:NoneDragCameraMove(handler)
    local inputVec = Input.mousePosition
    if inputVec.x < 0 or inputVec.x > 10000 then
        inputVec = Vector2(0, inputVec.y)
    end
    ...
    if self.prevMousePos then
        ...
        self.beginTime = self.beginTime or UTime.time
        if math.abs(self.beginTime - UTime.time) < DeltaTime * 4 then
            self.offVec = Vector3.zero
        end
        if clear then
...
-- 这个方法用于在场地中更新单位长度和初始位置
function ActivityIslandMapBehaviour:RefreshStartPosPointerDown(handler)
    local inputVec = Input.mousePosition
    if inputVec.x < 0 or inputVec.x > 10000 then
        inputVec = Vector2(0, inputVec.y)
    end
...
return ActivityIslandMapBehaviour