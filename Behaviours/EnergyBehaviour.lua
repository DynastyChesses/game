...
function EnergyBehaviour:Click(data)
    ...
    if self.behaviour then
		...
		ClientData:GetInstance():upEnergy(tonumber(self.behaviour.data.AddCount), nil, position, nil, true)
	end
	self.behaviour:DestroyMySelf()
end
return EnergyBehaviour