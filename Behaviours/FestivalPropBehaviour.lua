local FestivalPropBehaviour = class(BaseBehaviour, "FestivalPropBehaviour")
local ConstData = require "Game.Data.LocalData.ConstData"
local superMapUtils = SuperMapUtils.Instance
function FestivalPropBehaviour:Click(data)
    ...
    if self.behaviour then
    	...
    	local nameSplit = string.split(self.behaviour.gameObject.name, '_')
    	local cellId = nameSplit[1]
		-- ClientData:GetInstance():upCoin(tonumber(self.behaviour.data.AddCount), nil, position, nil, true)
		local festivalName = App:applyFunc(ConstData.Festival,  "CheckFestivalByCellId", cellId)
		-- debug.log(festivalName, string.sub(cellId,1, string.len(cellId) - 2), self.behaviour.data.AddCount, position)
		App:applyFunc(ConstData.Festival,  "ChangePropValue", festivalName, string.sub(cellId,1, string.len(cellId) - 2), self.behaviour.data.AddCount, position)
	end

	self.behaviour:DestroyMySelf()
end

return FestivalPropBehaviour