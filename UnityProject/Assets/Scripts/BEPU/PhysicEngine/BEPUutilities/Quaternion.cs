...
namespace BEPUutilities
{
    ...
    public struct Quaternion : IEquatable<Quaternion>
    {
        ...
        public static Fix64 Dot(Quaternion a, Quaternion b) => (a.X * b.X +  a.Y *  b.Y +  a.Z *  b.Z +  a.W *  b.W);
        public static Fix64 Angle(Quaternion a, Quaternion b)
        {
            Fix64 num = Quaternion.Dot(a, b);
            Fix64 value = Fix64.Abs(num) < 1m ? Fix64.Abs(num) : 1m;
            return Quaternion.IsEqualUsingDot(num) ? 0.0m : (Fix64) ((Fix64) Fix64.Acos(value) * 2.0m * 57.2957801818848m);
        }
        private static bool IsEqualUsingDot(Fix64 dot) =>  dot > 0.999998986721039m;
        ...
        {
            Fix64 num1 = rotation.X * 2m;
            Fix64 num2 = rotation.Y * 2m;
            Fix64 num3 = rotation.Z * 2m;
            ...
            Vector3 vector3;
            vector3.X = (Fix64) ((1.0m - ((Fix64) num5 + (Fix64) num6)) * (Fix64) point.X + ((Fix64) num7 - (Fix64) num12) * (Fix64) point.Y + ((Fix64) num8 + (Fix64) num11) * (Fix64) point.Z);
            vector3.Y = (Fix64) (((Fix64) num7 + (Fix64) num12) * (Fix64) point.X + (1.0m - ((Fix64) num4 + (Fix64) num6)) * (Fix64) point.Y + ((Fix64) num9 - (Fix64) num10) * (Fix64) point.Z);
            vector3.Z = (Fix64) (((Fix64) num8 - (Fix64) num11) * (Fix64) point.X + ((Fix64) num9 + (Fix64) num10) * (Fix64) point.Y + (1.0m - ((Fix64) num4 + (Fix64) num5)) * (Fix64) point.Z);
            return vector3;
        }
        ...
        public static explicit operator UnityEngine.Quaternion(Quaternion quaternion)//显式转换
        {
            return new UnityEngine.Quaternion((float)quaternion.X,(float)quaternion.Y,(float)quaternion.Z,(float)quaternion.W);
        }        
        public static explicit operator Quaternion(UnityEngine.Quaternion quaternion)//显式转换
        {
            return new Quaternion((Fix64)quaternion.x,(Fix64)quaternion.y,(Fix64)quaternion.z,(Fix64)quaternion.w);
        }
        ...
        {
            euler *= (3.14159265358979m / 180m);
            var x = Fix64.Sin(euler.Y / 2m) * Fix64.Sin(euler.Z / 2m) * Fix64.Cos(euler.X / 2m) +
                    Fix64.Cos(euler.Y / 2m) * Fix64.Cos(euler.Z / 2m) * Fix64.Sin(euler.X / 2m);
            var y = Fix64.Sin(euler.Y / 2m) * Fix64.Cos(euler.Z / 2m) * Fix64.Cos(euler.X / 2m) +
                    Fix64.Cos(euler.Y / 2m) * Fix64.Sin(euler.Z / 2m) * Fix64.Sin(euler.X / 2m);
            var z = Fix64.Cos(euler.Y / 2m) * Fix64.Sin(euler.Z / 2m) * Fix64.Cos(euler.X / 2m) -
                    Fix64.Sin(euler.Y / 2m) * Fix64.Cos(euler.Z / 2m) * Fix64.Sin(euler.X / 2m);
            var w = Fix64.Cos(euler.Y / 2m) * Fix64.Cos(euler.Z / 2m) * Fix64.Cos(euler.X / 2m) -
                    Fix64.Sin(euler.Y / 2m) * Fix64.Sin(euler.Z / 2m) * Fix64.Sin(euler.X / 2m);
