...
Shader "Custom/GaussionBlurEffect"
{
	//-----------------------------------【属性 || Properties】------------------------------------------    
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			VertexOutputBlit vert(VertexInputBlit v)
			{
				VertexOutputBlit o;
			...
			fixed4 frag(VertexOutputBlit i) : SV_Target
			{
				return fixed4(tex2D(_MainTex, i.tc0).rgb, 1);
			}
