...
/// 镂空背景图
/// </summary>
public class UIHollowOutImage : MaskableGraphic, ISerializationCallbackReceiver, ILayoutElement, ICanvasRaycastFilter
{
    ...
    [Header("是否实时刷新")] [Space(25)] public bool realtimeRefresh;
    [Header("是否显示镂空")] [Space(25)] public bool ShowHollowOut = true;
    ...
    protected override void OnPopulateMesh(VertexHelper vh)
    {
        ...
        for (int j = 0; j < commonPoint.Count; j += 2)
        {
            ...
            for (int i = 0; i < TriangleNum; i++)
            {
                ...
                pos2 = new Vector2(commonPoint[j].x + radius * Mathf.Cos(curDegree),
                    commonPoint[j].y + radius * Mathf.Sin(curDegree));
    ...
    protected Vector2[] GetTextureUVS(Vector2[] vhs, float tw, float th)
    {
        ...
        {
            uvs[i] = new Vector2(vhs[i].x / tw + 0.5f,
                vhs[i].y / th + 0.5f); //矩形的uv坐标  因为uv坐标原点在左下角，vh坐标原点在中心 所以这里加0.5（uv取值范围0~1）
        }
    ...
    public bool IsRaycastLocationValid(Vector2 screenPoint, Camera eventCamera)
    {
        ...
        return !(localPoint.x >= inner_lb.x && localPoint.x <= inner_rt.x &&
                 localPoint.y >= inner_lb.y && localPoint.y <= inner_rt.y);
    }
}