...
public class ETCJoystick : ETCBase,IPointerEnterHandler,IDragHandler, IBeginDragHandler, IPointerDownHandler, IPointerUpHandler {
	...
	public override void Start(){
        ...
        JoyPos = this.gameObject.transform.rectTransform().localPosition;
	...
	public override void Update (){
		...
		if (joystickType == JoystickType.Dynamic && _activated){
			...
			if (isTouchOverJoystickArea(ref localPosition, ref screenPosition)){
				...
				if (overGO == null || (allowJoystickOverTouchPad && overGO.GetComponent<ETCTouchPad>()) || (overGO != null && overGO.GetComponent<ETCArea>() ) ) {
					cachedRectTransform.localPosition = localPosition;
		...
		if (joystickType == JoystickType.Dynamic){
			if (GetTouchCount()==0){
                cachedRectTransform.localPosition = JoyPos;
            }
		}
		#endregion
		...
		WechatMiniGameEtcControl.instance.ClearTouchStart();
#endif
	}
...
#if RG_WECHAT_MINIGAME && !UNITY_EDITOR
		var needOnUp = WechatMiniGameEtcControl.instance.BeforeOnPointerEnter();
		if (needOnUp)
		{
			OnUp();
		}
	...
	public void OnDrag(PointerEventData eventData){
		...
		if (pointId == eventData.pointerId && GetTouchCount() > 0){
			isOnDrag = true;
			isOnTouch = true;
			...
			if (!isNoReturnThumb)
			{
				RectTransformUtility.ScreenPointToLocalPointInRectangle(cachedRootCanvas.rectTransform(),
					eventData.position, cachedRootCanvas.worldCamera, out var eventPos);
				thumbPosition = (eventPos - (Vector2)cachedRectTransform.localPosition);// / (cachedRootCanvas.rectTransform().localScale.x  ) ;
			}
			else{
	...
	private void UpdateJoystick(){
		...
		#region Unity axes
		if (enableKeySimulation && !isOnTouch && _activated && _visible && GetTouchCount() > 0)
        {
	...
	private bool isTouchOverJoystickArea(ref Vector2 localPosition, ref Vector2 screenPosition){
		...
		while (i<count && !touchOverArea){
			#if ((UNITY_ANDROID || UNITY_IOS || UNITY_WINRT || UNITY_BLACKBERRY || UNITY_WEBGL) && !UNITY_EDITOR)
#if RG_WECHAT_MINIGAME && !UNITY_EDITOR
			// 小游戏的手势状态不一定能保持完整1帧，所以有可能接收不到点击按下，特殊处理可以接收touchStart
			Touch touch = EventSystem.current.currentInputModule.input.GetTouch(i);
			if ((touch.phase == TouchPhase.Began || WechatMiniGameEtcControl.instance.IsTouchStart())){
	...
	private int GetTouchCount(){
		...
		var count = 0;
#if UNITY_WEBGL
			count = EventSystem.current.currentInputModule.input.touchCount;
#else
